...
CREATE OR ALTER PROCEDURE VERIFICA_SALARIO 
    @CD_CARGO INT,
    @CD_ESCOLARIDADE INT,
    @SALARIO NUMERIC(10,2)
AS
BEGIN
    ...
    DECLARE C_FAIXA CURSOR FOR 
        SELECT MENOR_VALOR, MAIOR_VALOR 
        FROM TB_FAIXASALARIAL 
        WHERE CD_CARGO = @CD_CARGO AND CD_ESCOLARIDADE = @CD_ESCOLARIDADE

    OPEN C_FAIXA
    FETCH NEXT FROM C_FAIXA INTO @MENOR_VALOR, @MAIOR_VALOR

    WHILE (@@FETCH_STATUS = 0)
    BEGIN
        IF (@SALARIO < @MENOR_VALOR OR @SALARIO > @MAIOR_VALOR)
        BEGIN
            PRINT 'VALOR FORA DA FAIXA DEFINIDA'
        END
